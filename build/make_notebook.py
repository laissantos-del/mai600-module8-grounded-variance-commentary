...
It runs on any machine that has the four things the next cell checks for: Python 3.10 or
newer, this repository, the packages in `requirements.txt`, and Ollama serving the model.
Nothing about it is specific to the machine the results were produced on.
...
# generation only, everything else runs without it
# install Ollama from https://ollama.com, then
ollama pull qwen3:4b-instruct
ollama serve
```

Then open this notebook from inside that folder and run all cells. The preflight below
names anything that is missing rather than failing obscurely, and the model server is the
only optional item: without it the generation cells announce that and skip, and the
evaluation reads the scored results already in `results/`.

**This notebook ships with the outputs of a previous run**, so every result is visible
before you execute anything. Restart the kernel and run all cells to see it work.

| Computed when you run it | Read from the committed evaluation |
|---|---|
| corpus loading and section counts | the improvement table, whose "before" half lives in the Module 7 repository |
| point-in-time admissibility | the three figures, generated by the scripts in `build/` |
| the variance engine and materiality | |
| embeddings, retrieval on both axes | |
| the sufficiency gate | |
| prompt assembly | |
| generation, when the model server is running | |
| the baseline ladder, recomputed from the per-case scores | |
| the retrieval comparison, re-derived from the corpus | |

The ladder and the retrieval comparison check themselves against the published files and
print whether they agree, so a disagreement is visible rather than silent.
""")

code('''
# ----------------------------------------------------------------- preflight
# Checks the environment first and names anything missing, rather than failing
# obscurely several cells later. The project runs on any machine satisfying these.
import importlib.util, sys
from pathlib import Path


def find_root(start: Path):
    """Nearest directory at or above `start` that holds src/gvc."""
...
if ROOT is None:
    problems.append("Cannot find the project. Open this notebook from inside the "
                    "cloned repository, so that src/gvc sits at or above the "
                    "working directory.")
else:
...
missing = [name for name, mod in [
    ...
    ("requests", "requests"), ("sentence-transformers", "sentence_transformers"),
    ("faiss-cpu", "faiss"),
...
if problems:
    for _p in problems:
        print("PREFLIGHT:", _p)
    raise RuntimeError("Preflight failed, see the lines above.")

print(f"python   : {sys.version.split()[0]}")
print(f"project  : {ROOT.name}")
print("packages : all present")

# The model server is optional, because most of the notebook does not need it.
from gvc.generate import DEFAULT_MODEL, ollama_available
OLLAMA_UP, ollama_msg = ollama_available(DEFAULT_MODEL)
print(f"model    : {ollama_msg}")
if not OLLAMA_UP:
    print("")
    print("  Generation will be skipped and the saved scores read instead.")
    print("  To run it, install Ollama from https://ollama.com, then:")
...
from gvc.generate import run_B3, run_B4
...
# re-derived here from the corpus rather than read from a file.
